...
# Predicting seasonal_allergies from microbiome data
...
```
# Random Forest
From here we will start with the actual setup of the model. We will set it up to predict whether a sample is classified as having seasonal_allergies using the measured genera. Question: Which output variables do we want to predict? Answer: Only yes and no.

For better convenience, we will combine the ASV table with the one column from the metadata which holds the information about seasonal_allergies (`seasonal_allergies` values).
```{r}
model_data <- taxonomic_table_only_measured |> 
  left_join(filter(tags,tag=="seasonal_allergies")) |>
  select(-X1) |>
  mutate(seasonal_allergies = value) |>
  select(-project,-srr,-value,-tag,-srs)

non_information_values <- c('not collected', 'not provided', 'unspecified')

samples_without_information <- model_data |>
  filter(is.na(seasonal_allergies) | seasonal_allergies %in% non_information_values) |>
  mutate(seasonal_allergies = as.factor(seasonal_allergies))

model_data <- model_data |>
  filter(!is.na(seasonal_allergies) & !(seasonal_allergies %in% non_information_values)) |>
  mutate(seasonal_allergies = as.factor(seasonal_allergies))

ggplot(model_data, aes(x=seasonal_allergies, fill=seasonal_allergies))+
  geom_bar()+
  theme_minimal()+
  ggtitle('Class distribution for seasonal_allergies')
```
## Prepare data splits

We want to split the data into training and testing. During data splitting, we have to take care that the proportion of seasonal_allergies stays equal between both data splits.
```{r}
set.seed(123)
split <- initial_split(data = model_data, 
                       prop = c(0.75), 
                       strata = seasonal_allergies)
...
```{r}
train_recipe <- recipe(seasonal_allergies ~ ., data = train_data) |>
  step_downsample(seasonal_allergies) # Possible but we want to create a screening model, so it is best to have a slight imbalance towards the positive class, step_downsample is from the package themis
...
show_best(tuning_results, metric = 'f_meas', n = 10)
```
...
final_fit <- final_workflow |> 
  last_fit(split, metrics = metric_set(accuracy, f_meas, roc_auc, sensitivity, specificity, precision, recall)) 

roc_df <- final_fit |> 
    collect_predictions() |> 
    dplyr::rename('prediction' = .pred_class) |>
    dplyr::rename('no' = .pred_yes) |>
    roc_curve(truth = seasonal_allergies, starts_with('.pred_'))
...
confusion_matrix <- final_fit |> 
  collect_predictions() |> 
  conf_mat(truth = `.pred_class`, estimate = seasonal_allergies)

autoplot(confusion_matrix, type = "heatmap") + 
    labs(
        title = "Confusion Matrix",
        x = "Predicted seasonal_allergies",
        y = "Actual seasonal_allergies"
    ) +
    theme_minimal()+
...
Remember the samples we removed earlier, because they had no definitive label regarding seasonal_allergies? It looks like our model generalized well enough that we can apply it now to predict the disease label of these samples using their microbial composition!

```{r}
predicted_labels <- predict(final_rf, new_data = samples_without_information)
samples_without_information$seasonal_allergies <- predicted_labels$.pred_class

ggplot(samples_without_information, aes(x=seasonal_allergies, fill=seasonal_allergies))+
  geom_bar()+
  theme_minimal()+
